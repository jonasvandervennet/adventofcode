...
def get_wire_intersections(w1, w2):
    ...
    intersections = [
        (position, i, w2_visited.index(position))
        for i, position in enumerate(w1_visited)
        if position != (0, 0) and position in w2_set]
    return intersections


def get_fastest_intersection(w1, w2):
    intersections = get_wire_intersections(w1, w2)
    best_intersection = None
    best_dist = None
    for intersection, idx_w1, idx_w2 in intersections:
        dist = idx_w1 + idx_w2
...
def main():
    ...
    for w1, w2, answer in test_values:
        closest_intersection, dist = get_fastest_intersection(w1, w2)
        assert(dist == answer)
    ...
    w1 = w1.split(',')
    w2 = w2.split(',')
    _, dist = get_fastest_intersection(w1, w2)
    print(dist)
