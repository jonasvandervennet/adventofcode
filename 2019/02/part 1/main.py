...
def execute_intcode(program):
    ...
    opcode 1: sum values at indices given by next to values, store at index of the third value
    opcode 2: multiply values at indices given by next to values, store at index of the third value
    opcode 99: halt
    """
    i = 0
    while i < len(program):
...
def main():
    ...
    test_inputs = [[[int(integer) for integer in program.split(',')] for program in line.split(' ')] for line in test_inputs]
    ...
    # cast read values to right format and type
    input_program = [[int(integer) for integer in program.split(',')] for program in inputs][0]
